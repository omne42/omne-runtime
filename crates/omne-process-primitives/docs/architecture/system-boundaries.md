...
- `resolve_command_path*` 这类 probe helper 对 `./tool`、`subdir/tool` 只按显式路径解释，
  不会继续把它们拿去扫描 `PATH`，以保持与 shell/`exec` 一致的命令解析契约；Unix 上单独的
  `\` 不会被误判成路径分隔符，Windows drive-relative 路径（例如 `C:tool.exe`）则继续按显式
  相对路径处理，不会误判成 bare command。
...
- Windows 下先等待 `taskkill /T /F` 的真实退出结果；只有它失败时才回退到 best-effort
  ...
  身份，只在当前快照仍匹配同一个 root 进程时才继续终止并在成功后消费掉这份句柄，避免 PID
  重用时误杀无关进程树，也避免 fallback 失败时把 leader 从后续重试机会里静默丢掉。
...
- general direct-execution 环境变量过滤；这里仍保留 direct command 的宿主原生 inherited-plus-request overlay env 语义，只把 sudo target 的环境重建成同一模型。
