...
- 探测命令是否存在和是否可执行。
- 运行宿主机命令并捕获输出，且 request/recipe 边界保留 `OsString` argv/env，而不是把低层原语锁死在 UTF-8 `String`。
- 当命中 `sudo` 路径时，把调用方显式提供的环境变量作为目标命令赋值参数继续传给目标命令，而不是只注入到 `sudo` 自身进程环境，也不在这个转发边界上强制做 UTF-8 转换。
