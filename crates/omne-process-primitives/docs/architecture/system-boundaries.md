...
- `sudo` 与 `env` 这类提权 helper 从可信标准位置解析，不跟随调用方请求里的 `PATH` 覆盖。
...
- Unix 上一旦无法重新验证原始 leader 身份，默认停止继续对该 process-group 做 `killpg`；Linux 只有在 capture 时拿到了完整 leader identity 后，才会在 `/proc` 中回扫同 session 的残留成员。
