...
fn resolve_env_program() -> PathBuf {
    resolve_trusted_helper_path(OsStr::new("env"), "/usr/bin/env")
}

fn sudo_available() -> bool {
    resolve_trusted_helper(OsStr::new("sudo")).is_some()
}

fn resolve_sudo_program() -> PathBuf {
    resolve_trusted_helper_path(OsStr::new("sudo"), "/usr/bin/sudo")
}

fn resolve_trusted_helper_path(command: &OsStr, unix_fallback: &str) -> PathBuf {
    resolve_trusted_helper(command)
        .unwrap_or_else(|| trusted_helper_fallback(command, unix_fallback))
}

fn resolve_trusted_helper(command: &OsStr) -> Option<PathBuf> {
    resolve_command_path_in_standard_locations_os(command)
}

fn trusted_helper_fallback(command: &OsStr, unix_fallback: &str) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = command;
        PathBuf::from(unix_fallback)
    }

    #[cfg(not(unix))]
    {
        command.to_os_string().into()
    }
}
...
mod tests {
    ...
    #[cfg(unix)]
    use super::resolve_sudo_program;
    #[cfg(unix)]
    use super::{
    ...
    #[cfg(unix)]
    #[test]
    fn sudo_build_command_ignores_request_path_override_for_helper_resolution() {
        let temp = tempfile::tempdir().expect("tempdir");
