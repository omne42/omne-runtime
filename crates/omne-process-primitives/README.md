...
- trusted helper resolution for `sudo` and `env`, so elevated helper selection does not depend on request-provided `PATH`
...
- Unix process-group cleanup that still fails closed on leader-PID reuse; Linux only reaps same-session orphaned descendants after a fully captured leader identity, instead of guessing after capture gaps
