...
- sudo-style escalation that resolves the `sudo` wrapper, privileged `env -i` scrubber, and target command from trusted host locations, executes those exact absolute paths, and pins the elevated target `PATH` to trusted standard directories while preserving non-`PATH` request env overlays, so elevated commands never depend on `sudo` `secure_path` surprises or request-controlled root search paths
