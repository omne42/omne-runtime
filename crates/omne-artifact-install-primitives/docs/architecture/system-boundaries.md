...
- 把直接二进制资产原子安装到目标路径，并按目标 advisory lock 串行化 direct binary 与 binary-from-archive 安装。
...
- 把 SHA 校验、archive 提取和 staged commit 这类阻塞工作隔离到 blocking install task，避免 Tokio worker 线程被长时间占用。
