...
- 识别支持的宿主 OS/arch 组合。
- 把宿主组合映射到 canonical target triple，并在 Linux 上优先根据当前进程实际加载的 loader/libc 证据区分 `gnu` / `musl` 宿主 ABI；只有完全拿不到这层证据时才回退到本地 ABI marker，若当前进程证据或本地 marker 出现 glibc 与 musl 并存、无法可靠判定的情况则直接 fail closed，不返回宿主平台。
