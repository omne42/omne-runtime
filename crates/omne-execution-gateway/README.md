...
- workspace boundary enforcement (`cwd` must stay inside `workspace_root`; `cwd` / `workspace_root` reject symlink or reparse-point ancestors before canonical binding, except for macOS root aliases such as `/var` and `/tmp`, execution revalidates the bound directory identities before spawn, and Unix spawn-time `cwd` changes use a pre-opened directory handle instead of a path-name `chdir`)
...
- on Unix, `cwd` revalidation now also carries a pre-opened directory handle into spawn, so the remaining OS-level TOCTOU note applies to executable path resolution rather than a second path-name lookup for `current_dir`.
