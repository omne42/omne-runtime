...
- `ExecGateway::new()`, `Default`, and `with_supported_isolation()` derive their default isolation policy from the host capability report instead of assuming `BestEffort`, so the default gateway remains usable on hosts that currently support only `None`.
...
- if `audit_log_path` is configured, preflight creates missing parent directories and rejects requests fail-closed when the audit log cannot be opened for append. Audit-log setup and policy-file loading both reuse the shared `omne-fs-primitives` no-follow regular-file helpers, so symlink/special-file rejection also applies across ancestor directories instead of only the final path component.
