...
- when `enforce_allowlisted_program_for_mutation = true`, callers must use an explicit absolute program path and must still set `with_declared_mutation(...)` intentionally instead of relying on the constructor default.
...
- shell-like, interpreter, and similar multiplexing frontends are denied outright because the gateway cannot safely authorize runtimes that can execute arbitrary scripts, package operations, or subcommands behind a single executable path.
- the gateway does not infer arbitrary tool-specific read/write semantics from executable basenames. Direct tools such as `git` or `cargo` may still be authorized explicitly through `non_mutating_program_allowlist`, but multiplexing launcher/frontend families such as `python*`, `pip*`, and `nodejs` are denied fail-closed because executable identity alone does not prove a read-only subcommand.
- mutation authorization now requires explicit absolute program paths in both the request and the relevant policy allowlist. Bare program names are denied fail-closed before mutation classification because they do not bind to a stable executable, and allowlist checks resolve by executable identity rather than basename text.
...
- those relative/drive-relative path rejections happen before mutation allowlist classification, so audit reasons stay stable as `relative_program_path_forbidden` instead of being misreported as a policy allowlist denial.
...
- `prepare_command()` now derives the returned `PreparedCommand` entirely from the audited `ExecRequest`; callers no longer pass a partially configured `Command`, so hidden `pre_exec`, `current_dir`, env, stdio, or other opaque process state cannot participate in spawn construction.
...
- if the last pre-spawn path or identity revalidation fails after preflight succeeded, the
