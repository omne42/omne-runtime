...
- fail-closed denial for known mutating tool families such as `git`, `make`, package managers, and core file-mutating utilities unless they are explicitly allowlisted
...
- known mutating tool families such as `git`, `make`, package managers, and core file-mutating utilities are also denied by default unless they are explicitly allowlisted, so callers cannot relabel obvious host-mutating tools as read-only and bypass the gate.
