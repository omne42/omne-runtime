...
- 配置了 `audit_log_path` 时，gateway 会在 preflight 阶段通过 descriptor-backed no-follow walk 逐层创建缺失父目录、拒绝祖先 symlink/reparse point，并验证日志可追加；如果审计日志不可用，请求必须 fail-closed 拒绝，而不是在无审计记录下继续执行。
