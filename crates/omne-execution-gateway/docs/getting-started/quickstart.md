...
- known mutating tools such as `git`, `make`, package managers, and `rm` -> denied when declared non-mutating; to run them, declare mutation and use an allowlisted explicit path.
