...
| `enforce_fs_tool_for_mutation` | `bool` | `true` | Requires caller-declared mutating requests to use allowlisted tools. |
| `fs_tool_program_allowlist` | `Vec<String>` | `omne-fs`, `omne-fs-cli` | Allowed program names or explicit paths for declared mutation. Basename matching is accepted for explicit paths. |
...
- mutation enforcement relies on caller declaration; the gateway does not parse `omne-fs` CLI syntax or infer arbitrary shell intent.
