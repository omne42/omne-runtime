...
| `enforce_allowlisted_program_for_mutation` | `bool` | `true` | Requires declared mutations to use allowlisted programs, and requires allowlisted mutating programs to set `declared_mutation = true`. |
...
3. Enforce mutation declaration consistency for allowlisted mutating programs.
...
- the gateway still does not parse arbitrary tool-specific CLI syntax or infer shell intent for non-allowlisted programs.
