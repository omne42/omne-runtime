...
- `omne-execution-gateway` 依赖 `omne-fs-primitives` 与 `policy-meta`。
  它复用前者提供的 ambient-root no-follow regular-file helper 来加载 policy/request/audit sink，
  同时把平台相关 sandbox 代码保留在 `src/sandbox/*` 下。
...
| `omne-execution-gateway` | 执行请求模型、隔离级别校验、`policy_default` 来源校验、执行时的 `workspace/cwd` 校验、声明式变更命令门控、sandbox 应用、审计事件与日志；并复用 `omne-fs-primitives` 的 low-level no-follow file helper 守住 policy/request/audit 的本地文件输入边界 | 文件系统操作策略、通用文件 API、`omne-fs` CLI 语义解析、超时/取消策略、stdout/stderr 保密策略、通用进程树清理原语 |
...
- no-follow 文件打开、ambient-root regular-file 校验与 bounded text read
