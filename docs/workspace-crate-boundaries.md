...
- 当前本仓库所有跨仓 foundation 依赖都固定到 manifest 里的 canonical git source pin，
  不允许回退成 sibling path dependency。
...
- `omne-process-primitives` 当前依赖 `omne-system-package-primitives`。
  它复用 canonical manager 解析来收口默认 `sudo` 模式推断与受信系统包命令识别，
  避免在命令执行 primitive 内并行维护第二份包管理器名称表；除此之外，它仍是独立的
  sibling crate，供需要通用命令探测、宿主机命令执行、host recipe 执行和进程树清理
  能力的下游 runtime 或 domain caller 直接依赖。
...
- 基于 `omne-system-package-primitives` canonical manager 解析的
...
它复用 `omne-system-package-primitives` 的 manager 枚举来避免边界漂移，但不接管 install
recipe、计划解释或产品级安装策略。
