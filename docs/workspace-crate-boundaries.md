...
本文说明这个 workspace 中七个 runtime crate 的职责划分：
...
- `omne-execution-gateway` 当前不依赖另外六个 workspace crate。
  它直接依赖 `policy-meta`，并把平台相关 sandbox 代码保留在 `src/sandbox/*` 下。
- `omne-process-primitives` 是独立的 sibling crate，供需要通用命令探测、宿主机命令执行和进程树清理能力的下游 runtime 或 domain caller 直接依赖。
...
也就是说，这个 workspace 是有意拆成“五类低层复用 crate”和“两类高层策略/编排 crate”。
随着 archive/compression 能力进入 workspace，这个原则没有变化：新增的是一个新的能力型
低层 crate，不是新的兜底桶。
...
| `omne-fs-primitives` | 无策略的文件系统原语：root materialization、capability 风格目录访问、no-follow open、symlink/reparse 分类、bounded read、advisory lock | `SandboxPolicy`、alias-root 语义、权限决策、secret 处理、redaction、CLI |
| `omne-host-info-primitives` | 无策略的宿主信息原语：宿主 OS/arch 识别、canonical target triple 映射、home 目录解析、目标可执行后缀推断 | `OMNE_DATA_DIR`/产品目录策略、包管理器适配、安装编排、CLI |
| `omne-integrity-primitives` | 无策略的完整性原语：`sha256:<hex>` 解析、原始 hex 输入解析、内容摘要计算与校验错误建模 | HTTP 下载、release 元数据、来源选择、安装编排、CLI |
| `omne-process-primitives` | 无策略的宿主机命令与进程原语：命令探测、带输出捕获的命令执行、Unix `sudo -n` 试探、process group、Linux `/proc` 身份校验、Windows Job Object、树形终止 helper | 命令 allowlist、环境变量过滤、超时/取消策略、领域错误映射、sandbox 选择 |
...
- `command_exists`、`command_available`、`command_path_exists`
- `run_host_command`
